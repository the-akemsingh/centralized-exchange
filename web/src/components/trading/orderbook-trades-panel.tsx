...
export function OrderbookTradesPanel({ symbol, ticker, depth, trades }: OrderbookTradesPanelProps) {
  ...
  return (
    <section className="w-full max-w-[560px] rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(16,20,32,0.96),rgba(10,14,24,0.98))] p-4 shadow-2xl shadow-black/25 backdrop-blur">
      <div className="flex items-center justify-between gap-3 border-b border-white/5 pb-3">
        ...
        <div className="rounded-2xl border border-white/10 bg-black/20 px-3 py-2 text-right">
          ...
          <div className="text-lg font-semibold text-slate-100">{ticker === null ? "--" : formatPrice(ticker)}</div>
        </div>
      </div>
      ...
      <div className="mt-4 inline-flex rounded-2xl border border-white/10 bg-black/20 p-1 text-sm">
        <button
          ...
          className={`rounded-xl px-4 py-2 font-semibold transition ${activeTab === "BOOK" ? "bg-white/10 text-slate-100" : "text-slate-400 hover:text-slate-200"}`}
        >
          Book
        </button>
        <button
          ...
          className={`rounded-xl px-4 py-2 font-semibold transition ${activeTab === "TRADES" ? "bg-white/10 text-slate-100" : "text-slate-400 hover:text-slate-200"}`}
        >
          Trades
        </button>
      </div>
      ...
      {activeTab === "BOOK" ? (
        ...
        <div className="mt-4">
          <div className="grid grid-cols-[1fr_0.7fr_0.7fr] gap-3 border-b border-white/5 px-2 pb-2 text-[11px] uppercase tracking-[0.25em] text-slate-500">
            <span>Price (INR)</span>
            <span className="text-right">Qty (TATA)</span>
            <span className="text-right">Side</span>
          </div>

          <div className="max-h-[420px] space-y-1 overflow-y-auto pt-2">
            {trades.length > 0 ? (
